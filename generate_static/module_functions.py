...
from get_attach_ext import get_ext
...
def add_test_tab(sn, card_id):
    ...
    print  					'<h2><u>Test Results for ngCCM %d</u></h2>' %sn
    print  				'</div>'
...
def ePortageTest(test_type_id, card_sn, test_name, revokes, desc_long):
    attempts =  Portage_fetch(test_type_id, card_sn) 
    print  			'<div class="row">'
    print           			'<div class="col-md-12">'
    print					'<h3> %(name)s </h3>' %{ "name":test_name}
    print					'<p> %s </p>' % desc_long
    print					'<br>'
...
#                        print "<td align=right style='{ background-color: yellow; }' ><a href='revoke_success.py?test_id=%(id)s'>Revoke</a></td>" %{ "id":attempt[5]}
        ...
        for afile in attachments:
            extension = get_ext(afile[0])
            print '<tr><td>Attachment: <a href="%s/attachments/get_attach_id%s%s">%s</a><td colspan=2><i>%s</i></tr>' % (webroot,afile[0],extension,afile[3],afile[2])
